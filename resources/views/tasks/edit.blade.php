            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('tasks.update',$task) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="deadline" class="block mt-1 w-full" type="date" name="deadline" :value="old('deadline',$task->deadline)" required />
                    ...
                    <div class="mt-4">
                        ...
                        <select name="user_id" id="user_id" class="block mt-1 w-full rounded border-gray-300">
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected(old('user_id',$task->user->id) == $user->id)>
                ...
                <div class="mt-4">
                    ...
                    <select name="client_id" id="client_id" class="block mt-1 w-full rounded border-gray-300 dark:text-white dark:bg-gray-700">
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}" @selected(old('client_id',$task->client->id) == $client->id)>
                                {{ $client->company_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    ...
                    <select name="project_id" id="project_id" class="block mt-1 w-full rounded border-gray-300 dark:text-white dark:bg-gray-700">
                        @foreach ($projects as $project)
                            <option value="{{ $project->id }}" @selected(old('project_id',$task->project->id) == $project->id)>
                                {{ $project->title }}
                            </option>
                        @endforeach
                    </select>
